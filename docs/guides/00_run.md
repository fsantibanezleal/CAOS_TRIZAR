...
A clone replays with **no torch and no Node sweep** — the trained ONNX + metrics + the engine `case-results.json`
are committed under `data/derived/`.
...
# 4) the SPA (copy-data overlays data/derived; live TS engine + ONNX)
